...
import { BadRequesError } from "../core/customError.js";

const loginUser: RequestHandler = asyncHandler(
  async (req: ProtectedRequest, res: Response) => {
    const { email, password } = req.body;
    const { success, data } = userLoginSchema.safeParse({ email, password });

    if (!success) {
      res.status(400);
      throw new BadRequesError("Invalid email or password");
    }
